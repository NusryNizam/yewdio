...
export class QueueService {
  private nowPlaying$ = new BehaviorSubject<DetailedAudioInterface | null>(
    null
  );
  ...
  isNext() {
    ...
    return (
      this.queue$.getValue().videoIDs.length - 1 !==
      this.queuePosition$.getValue()
    );
  }
  ...
  previousQueuePos() {
    ...
    if (currentPosition !== 0) this.queuePosition$.next(currentPosition - 1);
  }

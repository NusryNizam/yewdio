...
export class DataService {
  ...
  getSearchResults(searchTerm: string):Observable<AudioInterface[]> {
    ...
    return this.http.get<AudioInterface[]>(`${this._url}search?q=${searchTerm}`)
  } 

  getSearchResults2(searchTerm: string):void {
    searchTerm = searchTerm.trim().toLowerCase()
    this.searchTerm$.next(searchTerm)
    this.http.get<AudioInterface[]>(`${this._url}search?q=${searchTerm}`)
  ...
  getVideoDetails(videoId: string): Observable<DetailedAudioInterface> {
    return this.http.get<DetailedAudioInterface>(`${this._url}videos/${videoId}`)
  }
}
